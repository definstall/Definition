...
# Fail2ban 智能生命周期管理脚本
# 作者: 你的高级软件工程师
# 版本: 7.0 (Ubuntu专用, 引入配置移除功能, 修复MySQL日志预检逻辑)
# ==============================================================================
...
function initialize_environment() {
    ...
    if ! systemctl is-active --quiet fail2ban; then print_warn "服务未运行，正在尝试启动..."; systemctl start fail2ban; sleep 1; if systemctl is-active --quiet fail2ban; then print_success "服务启动成功！"; else print_error "启动失败，请手动执行 'sudo journalctl -u fail2ban' 查看日志。"; exit 1; fi; fi
...
function test_and_apply_config() {
    local config_content=$1; local target_file=$2; local temp_file="/etc/fail2ban/jail.d/temp-test-$(date +%s).local"
    print_info "正在进行配置预检测试..."; echo -e "$config_content" > "$temp_file"
    local test_output; test_output=$(fail2ban-client -t 2>&1); local test_status=$?; rm "$temp_file"
    if [ $test_status -ne 0 ]; then
        print_error "配置预检失败！您的服务和配置未作任何更改。"; print_warn "Fail2ban 报告了以下错误:"; echo -e "${C_YELLOW}$test_output${C_RESET}"; return 1
    else
        print_success "配置预检通过。"; backup_config "$target_file"; echo -e "$config_content" > "$target_file"; print_success "配置文件已创建于 $target_file"
        print_info "正在重新加载 Fail2ban..."; if fail2ban-client reload; then print_success "重新加载成功。"; else print_error "重新加载失败，请检查日志。"; fi; return 0
    fi
}
...
while true; do
    clear; echo -e "${C_CYAN}=============================================${C_RESET}"; echo -e "${C_CYAN}      Fail2ban 智能管理脚本 v7.0 (Ubuntu专用)      ${C_RESET}"; echo -e "${C_CYAN}=============================================${C_RESET}";
