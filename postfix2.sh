...
configure_firewall() {
    ...
    if [ "$OS_FAMILY" = "ubuntu" ]; then
        ...
        if ! command -v ufw >/dev/null 2>&1; then
            ...
            apt install -y ufw >> "$INSTALL_LOG" 2>&1 || print_error "UFW 安装失败！"
        fi
        ...
        if [ $? -eq 0 ]; then
            print_status "UFW 规则已应用并启用：允许 22, 25, 465, 587 端口。" true
        else
        ...
        if ! command -v netfilter-persistent >/dev/null 2>&1; then
            ...
            # 安装 netfilter-persistent 和 iptables-persistent
            apt install -y netfilter-persistent iptables-persistent >> "$INSTALL_LOG" 2>&1 || print_error "netfilter-persistent 安装失败！"
        fi

        # 添加规则前先检查，避免重复运行时堆积相同规则
        iptables -C INPUT -p tcp --dport 25 -j ACCEPT 2>/dev/null || iptables -A INPUT -p tcp --dport 25 -j ACCEPT >> "$LOG_FILE" 2>&1
        iptables -C INPUT -p tcp --dport 465 -j ACCEPT 2>/dev/null || iptables -A INPUT -p tcp --dport 465 -j ACCEPT >> "$LOG_FILE" 2>&1
        iptables -C INPUT -p tcp --dport 587 -j ACCEPT 2>/dev/null || iptables -A INPUT -p tcp --dport 587 -j ACCEPT >> "$LOG_FILE" 2>&1
        iptables -C INPUT -p tcp --dport 22 -j ACCEPT 2>/dev/null || iptables -A INPUT -p tcp --dport 22 -j ACCEPT >> "$LOG_FILE" 2>&1
        ...
        if [ $? -eq 0 ]; then
            print_status "IPTABLES 规则已应用并持久化：允许 22, 25, 465, 587 端口。" true
        else
...
show_header() {
    ...
    echo -e "${YELLOW}----------------------------------------------------${NC}"
    echo -e " 发信域名: ${GREEN}${DOMAIN:-未设置}${NC} | SMTP主机: ${GREEN}${MAIL_HOSTNAME:-未设置}${NC} | IP: ${GREEN}${EXTERNAL_IP:-未设置}${NC}"
    echo -e "${YELLOW}====================================================${NC}"
}
...
check_json_parser() {
    if command -v jq >/dev/null 2>&1; then
        ...
        print_warning "未找到 jq，脚本需要 jq 处理 Cloudflare JSON。正在安装，最多等待 180 秒..."
        print_status "APT 安装日志：$INSTALL_LOG"

        if ! timeout 180s bash -c 'apt-get update -y && apt-get install -y jq' &>> "$INSTALL_LOG"; then
            print_error "jq 自动安装失败或超时。请检查网络、APT 锁或手动执行：apt-get update && apt-get install -y jq"
            print_error "详细日志：$INSTALL_LOG"
            exit 1
        fi
        ...
        if command -v jq >/dev/null 2>&1; then
            ...
            print_error "无法安装 jq。脚本依赖 jq 生成和解析 Cloudflare JSON，请手动安装后重试。"
            exit 1
        fi
    fi
}
...
validate_variable() {
    local var_name="$1"
    local var_value="$2"
    if [[ "$var_value" == *$'\n'* || "$var_value" == *$'\r'* || "$var_value" == *\'* || "$var_value" == *\"* || "$var_value" == *\;* || "$var_value" == *'$'* || "$var_value" == *'`'* ]]; then
        print_error "$var_name 包含非法字符（换行、引号、分号、美元符号或反引号）：$var_value"
        exit 1
    fi

    case "$var_name" in
        DOMAIN|MAIL_HOSTNAME|parent|MAIN_DOMAIN)
            if [[ -n "$var_value" && ! "$var_value" =~ ^[A-Za-z0-9]([A-Za-z0-9.-]{0,251}[A-Za-z0-9])?$ ]]; then
                print_error "$var_name 不是有效域名格式：$var_value"
                exit 1
            fi
            ;;
        CLOUDFLARE_EMAIL)
            if [[ -n "$var_value" && ! "$var_value" =~ ^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$ ]]; then
                print_error "$var_name 不是有效邮箱格式：$var_value"
                exit 1
            fi
            ;;
        SMTP_USER)
            if [[ -n "$var_value" && ! "$var_value" =~ ^[A-Za-z0-9._-]+$ ]]; then
                print_error "$var_name 只能包含字母、数字、点、下划线和连字符：$var_value"
                exit 1
            fi
            ;;
    esac
...
safe_load_config() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            CLOUDFLARE_EMAIL|CLOUDFLARE_API_KEY|DOMAIN|MAIL_HOSTNAME|SMTP_USER|SMTP_PASS)
                ;;
            *)
                print_warning "忽略未知配置项：$key"
                continue
                ;;
        esac
...
save_config() {
    ...
    validate_variable "MAIL_HOSTNAME" "$MAIL_HOSTNAME"
    validate_variable "SMTP_USER" "$SMTP_USER"
    validate_variable "SMTP_PASS" "$SMTP_PASS"
...
cloudflare_api() {
    ...
    if [[ ! "$http_code" =~ ^[0-9]+$ ]]; then
        print_error "Cloudflare API 未返回有效 HTTP 状态码：$http_code"
        exit 1
    fi
...
get_zone_id() {
    ...
    # 从完整发信域名开始逐级向父域查找：
    # notice.example.com -> example.com。这样能兼容 Cloudflare zone 建在根域或子域的情况。
    while [ -n "$candidate" ] && [[ "$candidate" == *.* ]]; do
        print_status "正在 Cloudflare 中查找区域：$candidate"
        zones_response=$(cloudflare_api GET "zones?name=$candidate")
        zone_id_candidate=$(echo "$zones_response" | jq -r '.result[0].id // ""' 2>/dev/null || echo "")
        if [ -n "$zone_id_candidate" ]; then
            parent="$candidate"
            break
        fi
        candidate="${candidate#*.}"
    ...
    if [ -z "$zone_id_candidate" ]; then
        print_error "未找到匹配的 Cloudflare 区域。已尝试从 $DOMAIN 逐级查找父域，请检查 Cloudflare 账户权限。"
        exit 1
    fi
    ...
    print_status "已确认 Cloudflare 中存在匹配区域：$MAIN_DOMAIN，ID：$ZONE_ID"
}
...
add_dns_records() {
    ...
    # A 记录：SMTP 主机名 -> 当前服务器 IP
    response=$(cloudflare_api GET "zones/$ZONE_ID/dns_records?name=$MAIL_HOSTNAME&type=A")
    if [ "$USE_JQ" = true ]; then
...
PY
    ...
    json_data=$(jq -n --arg name "$MAIL_HOSTNAME" --arg ip "$EXTERNAL_IP" '{type: "A", name: $name, content: $ip, ttl: 120, proxied: false}')
    if [ -n "$record_id" ]; then
        print_status "更新 A 记录: $MAIL_HOSTNAME -> $EXTERNAL_IP" true
        cloudflare_api PUT "zones/$ZONE_ID/dns_records/$record_id" "$json_data" >/dev/null
    else
        print_status "创建 A 记录: $MAIL_HOSTNAME -> $EXTERNAL_IP" true
    ...
    if [ "$USE_JQ" = true ]; then
        record_id=$(echo "$response" | jq -r --arg mx "$MAIL_HOSTNAME" '.result[] | select(.content == $mx) | .id // ""' 2>/dev/null || echo "")
    else
        record_id=$(python3 - <<PY - 2>/dev/null
import json,os
d=json.loads(open(os.environ['TEMP_RESPONSE_FILE']).read())
...
PY
    ...
    json_data=$(jq -n --arg name "$record_name" --arg mx "$MAIL_HOSTNAME" '{type: "MX", name: $name, content: $mx, priority: 10, ttl: 120}')
    if [ -n "$record_id" ]; then
        print_status "更新 MX 记录: $DOMAIN -> $MAIL_HOSTNAME" true
        cloudflare_api PUT "zones/$ZONE_ID/dns_records/$record_id" "$json_data" >/dev/null
    else
        print_status "创建 MX 记录: $DOMAIN -> $MAIL_HOSTNAME" true
        cloudflare_api POST "zones/$ZONE_ID/dns_records" "$json_data" >/dev/null
    fi

    # SPF TXT
    local spf_value="v=spf1 mx ip4:$EXTERNAL_IP ~all"
    response=$(cloudflare_api GET "zones/$ZONE_ID/dns_records?name=$DOMAIN&type=TXT")
    if [ "$USE_JQ" = true ]; then
        record_id=$(echo "$response" | jq -r '.result[] | select(.content | startswith("v=spf1")) | .id // ""' 2>/dev/null | head -n1 || echo "")
    else
        record_id=$(python3 - <<PY - 2>/dev/null
import json,os
d=json.loads(open(os.environ['TEMP_RESPONSE_FILE']).read())
...
ensure_root() {
    if [[ $EUID -ne 0 ]]; then
        print_error "此脚本必须以 root 身份运行！请使用 sudo 或 root 用户执行。"
        exit 1
    fi
}

uninstall_all() {
    local cleanup_script_state="${1:-true}"

    print_status "正在停止并卸载所有邮件相关组件..."
    systemctl daemon-reload >/dev/null 2>&1 || true

    local svc
    for svc in postfix opendkim saslauthd; do
        if systemctl list-unit-files "${svc}.service" >/dev/null 2>&1 || systemctl list-units --all "${svc}.service" >/dev/null 2>&1; then
            print_status "正在停止 $svc 服务..." true
            systemctl stop "${svc}.service" >/dev/null 2>&1 || service "$svc" stop >/dev/null 2>&1 || true
        else
            print_status "$svc 服务未安装或未加载，跳过停止。" true
        fi
    done
    ...
    if [ "$cleanup_script_state" = "true" ]; then
        print_status "正在清理脚本配置文件与日志目录：$CONFIG_FILE, $LOG_DIR"
        rm -f "$CONFIG_FILE" || true
        rm -rf "$LOG_DIR" || true
    else
        init_logs
        print_status "重装流程保留脚本配置与日志目录。"
    fi

    systemctl daemon-reload >/dev/null 2>&1 || true
    if [ "$cleanup_script_state" = "true" ]; then
        echo -e "${GREEN}[信息]${NC} 所有邮件相关组件、脚本配置和日志已成功卸载/删除。" >&2
    else
        print_status "所有邮件相关组件已清理完成，准备继续重新安装。"
    fi
}
...
generate_self_signed_cert() {
    ...
    openssl genrsa -out "$KEY_FILE" 2048 &>> "$LOG_FILE"
    openssl req -new -key "$KEY_FILE" -out "$CERT_DIR/smtpd.csr" -subj "/C=US/ST=State/L=City/O=Self-Signed/CN=$MAIL_HOSTNAME" &>> "$LOG_FILE"
    openssl x509 -req -days 365 -in "$CERT_DIR/smtpd.csr" -signkey "$KEY_FILE" -out "$CERT_FILE" &>> "$LOG_FILE"
...
configure_postfix() {
    ...
    #含义：每次 SMTP 投递只带 1 个收件人，配合 rate_delay 让单域限速更接近按“封”计算。
    ...
    postconf -e "smtp_destination_rate_delay = 8s"
...
EOF
    ...
    print_status "正在更新 master.cf（幂等替换 smtp/submission/smtps 段）..."
    local master_tmp
    master_tmp=$(mktemp)
    awk '
        /^(smtp|submission|smtps)[[:space:]]+inet[[:space:]]/ { skip=1; next }
        skip && /^[[:space:]]/ { next }
        { skip=0; print }
    ' /etc/postfix/master.cf > "$master_tmp"
    cat "$master_tmp" > /etc/postfix/master.cf
    rm -f "$master_tmp"

    # 追加标准可靠的 smtp/submission/smtps 段（使用 'MASTER_EOF' 以防变量展开）
...
configure_logrotate_legacy_unused() {
...
EOF
    ...
    if ! printf '%s\n' "$SMTP_PASS" | saslpasswd2 -p -c -u "$DOMAIN" "$SMTP_USER" &>> "$LOG_FILE"; then
        print_error "SMTP SASL 账户创建失败，请检查 saslpasswd2 和 /etc/sasldb2 权限。"
        exit 1
    fi
...
EOF
    ...
    chmod 0750 /var/run/saslauthd 2>/dev/null || true
...
ExecStartPre=-/bin/chmod 0750 /var/run/saslauthd
ExecStartPost=-/bin/chgrp sasl /var/run/saslauthd/mux
ExecStartPost=-/bin/chmod 0770 /var/run/saslauthd/mux
...
EOF
    ...
    # 修复 saslauthd socket 权限，避免 submission/smtps 认证时报 Permission denied。
    if [ -S /var/run/saslauthd/mux ]; then
        chgrp sasl /var/run/saslauthd/mux 2>/dev/null || true
        chmod 0770 /var/run/saslauthd/mux 2>/dev/null || true
        print_status "saslauthd socket 权限已修正为 root:sasl 0770。"
    else
        print_warning "未发现 /var/run/saslauthd/mux，saslauthd 可能未正常启动。"
    fi
...
configure_opendkim() {
    ...
    mkdir -p /etc/dkimkeys/"$DOMAIN" || true
    echo "$DKIM_SELECTOR._domainkey.$DOMAIN $DOMAIN:$DKIM_SELECTOR:/etc/dkimkeys/$DOMAIN/$DKIM_SELECTOR.private" > /etc/opendkim/KeyTable || true
    chmod 644 /etc/opendkim/KeyTable || true

    echo "*@$DOMAIN $DKIM_SELECTOR._domainkey.$DOMAIN" > /etc/opendkim/SigningTable || true
    ...
    echo "$MAIL_HOSTNAME" >> /etc/opendkim/TrustedHosts || true
...
EOF
    ...
    cat >/etc/systemd/system/opendkim.service.d/local_override.conf <<'EOF'
[Service]
User=opendkim
Group=opendkim
EOF
...
EOF
    ...
    print_status "Logrotate 配置已写入 $logrotate_config，邮件日志将按 10MB/每日滚动。" true
}

optimizing_system(){
    print_warning "已禁用原脚本的激进系统优化：不会覆盖 /etc/security/limits.conf、不会开启 IP 转发、不会清空系统日志。"
    print_status "默认跳过保守文件句柄优化。如需高并发调优，请手动配置 /etc/sysctl.d 和 /etc/security/limits.d。"
}


# 新增：配置定时清理任务
setup_cron_maintenance() {
    print_warning "自动清空日志并重启 Postfix 的维护 cron 已禁用；请使用 logrotate 管理日志。"
}
...
install_postfix() {
    ...
    print_warning "防火墙配置会修改当前服务器入站规则；如果 SSH 不在 22 端口，请先手动放行你的 SSH 端口。"
    read -p "是否由脚本自动配置防火墙，放行 22/25/465/587 端口？(Y/n) [Y]: " firewall_confirm
    firewall_confirm=${firewall_confirm:-y}
    if [[ "$firewall_confirm" =~ ^[yY]$ ]]; then
        configure_firewall
    else
        print_status "已跳过自动防火墙配置。请确认 25/465/587 以及你的 SSH 端口已按需放行。"
    fi
    ...
    get_external_ip
    get_zone_id
    if [[ "$MAIL_HOSTNAME" != "$MAIN_DOMAIN" && "$MAIL_HOSTNAME" != *".$MAIN_DOMAIN" ]]; then
        print_error "SMTP 主机名 $MAIL_HOSTNAME 不在 Cloudflare 区域 $MAIN_DOMAIN 下，无法自动创建 DNS。"
        exit 1
    fi
    ...
    local dkim_dir="/etc/dkimkeys/$DOMAIN"
    mkdir -p "$dkim_dir"
    cd "$dkim_dir"

    if ! opendkim-genkey -b 2048 -d "$DOMAIN" -s "$DKIM_SELECTOR" &> /dev/null; then
    ...
    local dkim_name="$DKIM_SELECTOR._domainkey.$DOMAIN"
    ...
    systemctl reload postfix &>> "$LOG_FILE" || service postfix reload &>> "$LOG_FILE" || print_warning "Postfix reload 失败，请检查服务状态。"
    ...
    print_status "安装流程结束。若你选择了文件句柄优化，重新登录或重启后会对新会话完全生效。"
}
...
view_smtp_details() {
    ...
    echo -e "SMTP 主机: ${GREEN}${MAIL_HOSTNAME}${NC}"
    echo -e "发信域名: ${GREEN}${DOMAIN}${NC}"
...
get_user_input() {
    ...
    check_json_parser
    print_status "初步检测完成，开始加载/收集 Cloudflare 与域名配置。"
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        print_status "已加载配置：Email: ${CLOUDFLARE_EMAIL:-未设置}, Domain: ${DOMAIN:-未设置}, SMTP主机: ${MAIL_HOSTNAME:-未设置}"
    else
        print_warning "未找到配置文件 $CONFIG_FILE，需要首次输入 Cloudflare 信息。"
    fi
    ...
    if [ -z "$CLOUDFLARE_API_KEY" ]; then
        read -p "请输入您的 Cloudflare API 密钥（明文显示，便于核对）: " CLOUDFLARE_API_KEY
        needs_save=true
    fi
    if [ -z "$DOMAIN" ]; then
        read -p "请输入发信域名 (例如：example.com 或 notice.example.com): " DOMAIN
        needs_save=true
    fi
    if [ -z "$MAIL_HOSTNAME" ]; then
        MAIL_HOSTNAME="mail.$DOMAIN"
        print_status "SMTP 主机名/MX 目标已自动设置为：$MAIL_HOSTNAME"
        needs_save=true
    fi
    ...
    if $needs_save || [ -z "$SMTP_USER" ] || [ -z "$SMTP_PASS" ] || [ -z "$MAIL_HOSTNAME" ]; then
        save_config
    fi
    ...
    hostnamectl set-hostname "$MAIL_HOSTNAME" &> /dev/null || true
    print_status "主机名已设置为 $MAIL_HOSTNAME" true
}
...
restart_service_checked() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        systemctl restart "$service_name.service" 2>>"$LOG_FILE" || service "$service_name" restart 2>>"$LOG_FILE" || {
            print_error "$service_name 服务重启失败，请检查 systemctl status $service_name.service。"
            return 1
        }
        systemctl is-active --quiet "$service_name.service" 2>/dev/null || print_warning "$service_name 服务未报告 active 状态，请手动检查。"
    else
        service "$service_name" restart 2>>"$LOG_FILE" || {
...
restart_all_services() {
    ...
    restart_service_checked opendkim
    restart_service_checked saslauthd
    restart_service_checked postfix
...
postfix_management_menu() {
    while true; do
        ...
        local dkim_public_key_path="/etc/dkimkeys/$DOMAIN/$DKIM_SELECTOR.txt"
