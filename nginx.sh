...
function addDomainPort() {
    ...
    echo -e "    proxy_set_header Upgrade \$http_upgrade;" >> "$nginx_domain_conf_path"
    echo -e "    proxy_set_header Connection \"upgrade\";" >> "$nginx_domain_conf_path"
    echo -e "    proxy_read_timeout 86400s;" >> "$nginx_domain_conf_path"
...
function deleteDomainPort() {
    ...
    if [[ "$domain_name" == "c" ]]; then
        ...
        if [[ -n "$line_number" ]]; then
            ...
            end=$((line_number + 21))    # 删除之后的 20 行（包括可能的 location 配置）
